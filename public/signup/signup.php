    <?php
        require_once '../../dbconnect.php';
        require_once '../ValidateEmail.php';

        session_start();

        if (isset($_SESSION['signup_status'])) {
            echo "<div id='signup-status'>".$_SESSION['signup_status']."</div>";
            unset($_SESSION['signup_status']); // Show once, then remove
        }

        if (isset($_SESSION['otp'])) {
            unset($_SESSION['otp']);
        }
        
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            // Get the submitted username, password, and email
            $submitted_username = $_POST['username'];
            $submitted_password = $_POST['password'];
            $submitted_email = $_POST['email'];
        
            // Validate email address using your email validation function
            $email_response = new ValidateEmail($submitted_email);
            $emailStatus = $email_response->emailStatus();
        
            // If email is not valid, set session and redirect without inserting
            if ($emailStatus !== true && $emailStatus !== "") {
                $_SESSION['signup_status'] = 'invalid';
                header("Location: " . $_SERVER['PHP_SELF']);
                exit(); // Stop script here
            }
        
            // Check if the email already exists in the database
            $check_duplicate_query = "SELECT * FROM `user_credentials` WHERE `email` = ?";
            
            // Use prepared statement to prevent SQL injection
            if ($stmt = mysqli_prepare($conn, $check_duplicate_query)) {
                mysqli_stmt_bind_param($stmt, "s", $submitted_email); // "s" for string parameter
                mysqli_stmt_execute($stmt);
                $result = mysqli_stmt_get_result($stmt);
        
                // If email exists, set the session to 'duplicate'
                if (mysqli_num_rows($result) > 0) {
                    $_SESSION['signup_status'] = 'duplicate';
                } 
                else {
                    // Insert new user credentials if email is unique
                    $insert_query = "INSERT INTO `user_credentials` (`user_name`, `email`, `password`) VALUES (?, ?, ?)";
        
                    // Use prepared statement to prevent SQL injection
                    if ($stmt_insert = mysqli_prepare($conn, $insert_query)) {
                        // Hash the password before inserting it (optional, but recommended)
                        $hashed_password = password_hash($submitted_password, PASSWORD_DEFAULT);
                        
                        // Bind parameters and execute the query
                        mysqli_stmt_bind_param($stmt_insert, "sss", $submitted_username, $submitted_email, $hashed_password);
                        mysqli_stmt_execute($stmt_insert);
        
                        // Set success status
                        $_SESSION['signup_status'] = 'success';
                    } else {
                        // Handle potential errors with prepared statement
                        $_SESSION['signup_status'] = 'error';
                    }
                }
        
                // Close the statement
                mysqli_stmt_close($stmt);
            } else {
                // Handle failure to prepare the query
                $_SESSION['signup_status'] = 'error';
            }
        
            // Redirect to avoid form resubmission
            header("Location: " . $_SERVER['PHP_SELF']);
            exit(); // Ensure script stops after redirect
        }
        
    ?>

    <!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Signup</title>
        <link rel="stylesheet" href="../../style.css">
        <script src="https://code.jquery.com/jquery-3.7.1.js" integrity="sha256-eKhayi8LEQwp4NKxN+CfCh+3qOVUtJn3QNZ0TciWLP4=" crossorigin="anonymous"></script>
        <script src="./signup.js"></script>
        <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    </head>
    <body>
        <div class="container">
            <form action="" method="post" id="signup-form">
                <h1>Signup!</h1>
                <label>Enter your Username<span>*</span></label>
                <input type="text" name="username">
                <div class="message-box">
                    <p class="username-message"></p>
                </div>
                <label>Enter your Email Id<span>*</span></label>
                <div class="input-btn-wrapper">
                    <input type="text" name="email">
                    <button type="button" id="sendOtp-btn" class="otp-btn">Send OTP</button>
                </div>
                <div class="message-box">
                    <p class="email-message"></p>
                </div>
                <label>Enter the OTP you received<span>*</span></label>
                <div class="input-btn-wrapper">
                    <input type="text" name="otp">
                    <button type="button" id="verifyOtp-btn" class="otp-btn">Verify OTP</button>
                </div>
                <div class="message-box">
                    <p class="email-message"></p>
                </div>
                <label>Enter your Password<span>*</span></label>
                <input type="password" name="password">
                <div class="message-box">
                    <p class="password-message"></p>
                </div>
                <label>Confirm your Password<span>*</span></label>
                <input type="password" name="confirm-password">
                <div class="message-box">
                    <p class="confirm-password-message"></p>
                </div>
                <button type="submit">Submit</button>
            </form>
            <div class="btn-wrapper">
                <a href="../login/login.php" title="Login">Login</a>
            </div>
        </div>
    </body>
    </html>
